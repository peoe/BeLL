...
public class Pin implements ScadObject {
	...
	//returns lowerBaseTille -> Cylinder with small height placed at the bottom of the pin 
	private ScadObject getBaseTileLow(double l) {
		//calculates Radius out of given length and an addition of the positive Pin Radius
		double r = l + Params.getPinPRadius();
		ScadObject baseTile;
		// returns small cylinder and enlarges it by the EPSILON value 
		baseTile = new Cylinder(Params.getBasePlatePinCircleHeight(), r, true).resize(getEpsilon(), getEpsilon(), getEpsilon());	
		//translates cylinder by a half of its height 
		return (new Translate(baseTile, 0, 0, 0.5*Params.getBasePlatePinCircleHeight()));
	}
	
	//calculates the necessary length of a pin so it has a minimum distance to both walls
	...
	private ScadObject getPin() {
		...
		ScadObject pinCylinder;
			...
			baseCube = (new Cube(d, Params.getPinPWidth(), totalHeight, true)).resize(0.0, 0.0,
					getEpsilon(), getEpsilon(), 0.0, getEpsilon());
			//round cylinder segment
			pinCylinder =  new Cylinder(totalHeight, Params.getPinPRadius(), true).resize(getEpsilon(), 0.0,
							getEpsilon());
	...
	public ScadObject getPinObject() {
		...
		pinIntersection.add(getBaseTileLow(pinLength));
