...
Die Methode \icode{toString()} zum Beispiel stellt in den Klassen des Interfaces die Übergabe des OpenSCAD Befehlsstrings dar.
...
Die Summe aller Objekte des Typs \icode{CornerPin} repräsentieren den unteren Abschnitt des Eckpfeilers, welcher die Fixierung der Grundplatten gewährleistet.
...
Falls ein Eckpin für das äußere Gebiet berechnet werden soll, wird anstattdessen eine Differenzmenge zwischen der Umrandungsfläche des Grundrisses und dem flachen Zylinder gebildet.
...
Jede Grundplatte, die an das äußere Gebiet angrenzt wird zudem an den außen liegenden Seite um die halbe Wanddicke vergrößert, damit ein abgeschlossener Eindruck des gesamten 3D-Modells entsteht (siehe Abbildung \thebildnrnext).
...
Sie werden bei Beginn des Programms in der \icode{Main}-Klasse gesetzt und können über eine \icode{Params}- Objekt aufgerufen werden. \\
...
\begin{tabularx}{\textwidth}{|X|X|Z|}
	...
	\hline 
	\makecell{basePlatePinCircle-\\Height} & Höhe der Kreissegmenten an den Eckpins & 1 \\ %HEEELP
	\hline 
